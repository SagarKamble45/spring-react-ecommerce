...
public class CartServiceImpl implements CartService{
    ...
    public CartDTO addProductToCart(Long productId, Integer quantity) {
        ...
        //reduce the quantity when product selected
...
//        CartDTO cartDTO = modelMapper.map(cart, CartDTO.class);
    ...
    @Override
    public List<CartDTO> getAllCars() {
        ...
        List<CartDTO> cartDTOS = carts.stream()
                .map(item ->{
                    ...
                    List<ProductDTO> products = item.getCartItems().stream().map(
                            cartItem -> {
                                ProductDTO productDTO = modelMapper.map(cartItem.getProduct(), ProductDTO.class);
                                productDTO.setQuantity(cartItem.getQuantity()); // to manage quantities
    ...
    public CartDTO updateProductQuantityInCart(Long productId, Integer quantity) {
        ...
        Cart cart = cartRepository.findById(cartId).orElseThrow(()-> new ResourceNotFoundException("Cart", "cart",cartId));
    ...
    public String deleteProductFromCart(Long cartId, Long productId) {
        Cart cart = cartRepository.findById(cartId).orElseThrow(()->new ResourceNotFoundException("Cart", "carId", cartId));
        CartItem cartItem = cartItemRepository.findCartItemByProductIdAndCartId(cartId, productId);
        if (cartItem == null){
